...
```{r}
ps_model = glm(statin_use ~ age + region + asthma + cancer + cerebrovascular_disease + chronic_kidney_disease + heart_failure + tobacco_use + obesity + mi + chronic_pulmonary_disease + depression + diabetes + hyperlipidemia + hypertension + coronory_artery_disease + sex + race, data = sta, family = binomial)
...
However, after applying IPTWs to the Cox model, the association between statin use and hazard of the composite event is sharply attenuated.  Note that when using wewights, we need to use the "robust" estimator of the SE.
```{r}
